...
from server.models.models_lump import Board, Session
...
async def get_paging_widget(selected_page:int=0, span:int=3, skip=10) -> 'str':
    ...
    return Markup(await render_template('tpl_widget_paging.html', pages=range(start, count), start_page=selected_page + 1, skip=skip))

async def get_boards_all(session=None):
    auto_close = False
    if session is None:
        session = Session()
    ...
    out = Markup(await render_template('tpl_widget_boards_all.html', boards=items))
    if auto_close:
        session.close()
    return out
